...
To generate a **custom path** use the following command: `observation, info = env.reset(custom=[[Cx, Cy, Cz], a, b, o, j, w, [[T1x, T1y, T1z], [T2x, T2y, T2z]]])` for example `observation, info = env.reset(custom=[[3, 4, 5], 5, 4, 11, 12, 13, []])` where *[Cx, Cy, Cz]* are the coordinates of the centeral point of the ellipse, *a* is the semi-major axis of the ellipse, *b* is the semi-minor axis of the ellipse, and *o*, *j*, *w* are the angle orientations of the ellipse with values 0° to 90°. Then finally the list of the target coordinates, *[T1x, T1y, T1z]* is the coordinates of target 1, *[T2x, T2y, T2z]* is the coordinates of target 2. This command will automatically freeze the random seed to the value of 0.
